...
그래서 JS는 단순히 함수 호출을 스택에 쌓는 것만으로는 실행 흐름을 관리할 수 없습니다.  
코드가 "어떻게 실행되는지"까지 완전히 기술하는 구조의 실행 컨텍스트가 필요합니다.  
즉, 실행 컨텍스트도 스택 구조이지만 단순한 **스택 프레임**만으로는 부족하고, **함수마다의 스코프, 변수 상태, this, 외부 환경 참조까지** 모두 담을 수 있는, 더 정교한 실행 단위인 **실행 컨텍스트**가 필요하게 된 거죠.
