...
**"왜 재사용하려고 만든 컴포넌트가, 시간이 지날수록 재사용하기 힘들어지는 걸까?"**

재사용 가능한 컴포넌트는 초기 개발 속도를 높여주고 UI 일관성을 지켜주는 강력한 도구입니다. 하지만 서비스가 성장하고 요구사항이 고도화될수록, 이 컴포넌트들은 수많은 Props와 내부 분기 처리로 인해 점점 비대해지기 십상입니다.

저 또한 프로젝트를 진행하며 공통 컴포넌트에 Props가 계속 늘어나면서 인터페이스가 복잡해지는 문제를 겪었고, 새로운 기능을 추가할 때 기존 코드에 영향을 주는 경우도 마주했었습니다.

재사용 가능한 코드를 만들 때 흔히 겪는 문제:

1. **처음엔 간단하게 시작** → 재사용 가능한 함수/컴포넌트를 만듦
    
2. **새로운 요구사항** → 살짝 다른 use case가 생김
    
3. **옵션 추가** → prop/argument/option을 추가해서 대응
    
4. **반복** → 이 과정이 계속 반복됨
    

결국 사용하기도 유지보수하기도 어려운 코드가 됨

그러다 문득 "이게 과연 좋은 추상화일까?"라는 의문이 들었습니다.  
이 물음에 대한 답을 찾던 중, [Kent C. Dodds의 Inversion of Control(제어의 역전)](https://kentcdodds.com/blog/inversion-of-control#enter-inversion-of-control) 아티클을 접하게 되었습니다. 이 글을 통해 컴포넌트 설계에 대한 새로운 접근 방식을 고민해 볼 수 있었습니다.  
...
보통 우리는 함수나 컴포넌트를 만들 때, "모든 경우의 수를 내가 다 처리해 놔야지!"라고 생각합니다.  
...
물론, 모든 컴포넌트에 제어 역전 패턴을 적용하는 것이 정답은 아닙니다. 재사용성이 낮거나 단순한 컴포넌트에 무리하게 패턴을 도입하는 것은 오히려 불필요한 복잡도를 높이는 '성급한 추상화'가 될 수 있기 때문입니다.
